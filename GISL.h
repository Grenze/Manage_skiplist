...
public:
    ...
    void insert(const Value& l, const Value& r, uint64_t timestamp = 0);
...
template<typename Value, class Comparator>
void IntervalSkipList<Value, Comparator>::insert(const Value &l, const Value &r, uint64_t timestamp) {
    uint64_t mark = (timestamp == 0) ? timestamp_++ : timestamp;
    Interval* I = new Interval(l, r, mark);
    insert(I);
}
...
Interval::Interval(const Value &inf,
                   const Value &sup,
                   const uint64_t &stamp)
        : inf_(inf), sup_(sup), stamp_(stamp) {
    //assert( !(inf_ > sup_) );
}
